...
Akkadian is one of the oldest known written languages, used across Mesopotamia for over two millennia and preserved on hundreds of thousands of clay tablets. Despite this vast corpus, only a fraction of surviving texts have been translated — the bottleneck is the small number of trained Assyriologists worldwide. This project builds a neural machine translation system to help bridge that gap, translating Akkadian cuneiform into English.
...
1. **GLOSS** — Parse each token: identify logograms, bound morphemes, determinatives, and map them to their Akkadian readings and English meanings
2. **COMPOSE** — Build noun phrases, verb phrases, and clauses, annotating grammatical function (stem, tense, person, case)
3. **ASSEMBLE** — Compose the final English translation from the parsed components

We distill these on-policy, keeping only high-scoring examples where the reasoning is "load-bearing." The filtering criterion is strict: if Opus cannot mechanistically map the transliteration to the target translation without introducing unbounded terms — i.e. words that aren't justified by the source and lexicon — the example is discarded. Starting from 140,689 lines in the [ORACC](http://oracc.org/) corpus, we filtered down to ~26,000 training pairs (under 20%), using data quality as a feature rather than a cost.

The model is prompted with the source dialect (currently Neo-Assyrian) to disambiguate terms that differ across dialects, enabling broad Akkadian structural transfer while preserving dialect-specific meanings.

The final model is a full-parameter fine-tune of Qwen3-4B on these enriched training examples.
...
<!-- Placeholder: test set example -->
*[Test set example placeholder]*

## Links

- [Model on HuggingFace](#) *(coming soon)*
